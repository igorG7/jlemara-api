...
class UnitController {
  async createTemp(req: Request, res: Response) {
    try {
      ...
      return res.status(201).json({ message: "Unidade criada.", data: unit });
    } catch (error) {
      return res.status(500).json({ message: "Erro interno inesperado.", error });
    }
  }

  async findAvailables(req: Request, res: Response) {
    try {
      ...
      const [units, total] = await Promise.all([
        ...
        Unit.countDocuments({ unit_status: 0 }),
      ]);
  ...
  async findUnits(req: Request, res: Response) {
    try {
      ...
      Console({ type: "log", message: "Buscando unidades." });

      const [units, total] = await Promise.all([
        Unit.find(query, {}, { limit, skip: (page - 1) * limit }).lean(),
  ...
  async addPhotos(req: Request, res: Response) {
    try {
      ...
      if (!updatedUnit) {
        Console({ type: "error", message: "Unidade não encontrada para adicionar fotos." });
  ...
  async updatePhoto(req: Request, res: Response) {
    try {
      ...
      Console({ type: "success", message: "Foto de unidade atualizada com sucesso!" });
